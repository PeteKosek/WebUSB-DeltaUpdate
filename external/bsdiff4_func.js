...
function qsufsort(I, V, old, oldsize){
    var buckets = [256], i, h, len;
    for (i = 0; i < 256; i++)
        buckets[i] = 0;
    for (i = 0; i < oldsize; i++)
        buckets[old[i]]++;
...
function search( I, old, oldsize, neww, newsize, st, en, pos){
    var x, y;
    if (en - st < 2) {
        x = matchlen(old + I[st], oldsize - I[st], neww, newsize);
        y = matchlen(old + I[en], oldsize - I[en], neww, newsize);
        if (x > y) {
            pos = I[st];
            return x;
        } else {
    ...
    if (memcmp(old + I[x], neww, Math.min(oldsize - I[x], newsize)) < 0) {
        return search(I, old, oldsize, neww, newsize, x, en, pos);
    } else {
        return search(I, old, oldsize, neww, newsize, st, x, pos);
    }
}
...
*/
function diff(self, args){
    var lastscan, lastpos, lastoffset, oldscore, scsc, overlap, Ss, lens;
    var I, V, dblen, eblen, scan, pos, len, s, Sf, lenf, Sb, lenb, i;
    var controlTuples, tuple, results, temp;
    var origDataLength, newDataLength;
...
function encode_int64(self, value){
    var x, i;
    var bs=[8], sign = 0x00;
...
function decode_int64(self, string){
    var x, bs, i;
